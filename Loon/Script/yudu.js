...
async function login() {
  try {
    let loginurl =
      `https://${host}/member.php?mod=logging&action=login&loginsubmit=yes&handlekey=login&loginhash=LCr9B&inajax=1`;
    data = `formhash=adf03b6e&referer=https%3A%2F%2Fbbs.cloudepay.cc%2F&loginfield=username&username=bgcode&password=Kd4*%24As9gHH3N&questionid=0&answer=`
    let headers = {
      Host: `${host}`,
      referer: `http://${host}/`,
...
async function info() {
  try {
    ...
    await $.http.get(myRequest).then((res) => {
      let message = res.body.match(/<li><em>金币<\/em>(.+?) 枚<\/li>/);
      if (message) {
        $.result += "金币：" + message[1];
      }
      return true;
